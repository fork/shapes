module Shapes
  ...
  TYPE_NOT_ALLOWED_WARNING = 'Type \'%s\' not allowed in this Array'
  ...
  def self.extend_model(base) #:nodoc:
    if options = @config[base.name]
      ...
      base.class_eval %Q"def to_shapes_xml(options = {}, &block)
        ...
        block_given? ? serializer.build_xml(&block) : serializer.build_xml
      end"
