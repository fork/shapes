module DuckDescribe  
  module Primitives
    class File < DuckDescribe::Primitive
      ...
      def relative_path
        ::File.join(DuckDescribe::FILE_DIR, @value)[/\/public(\/.*)/, 1]
      end

      def absolute_path
        ::File.join(DuckDescribe::FILE_DIR, @value)
      end
      ...
      def create_tmp_file(file_obj)
        ...
        tmp_file_path = ::File.join DuckDescribe::FILE_TMP_DIR, @file_dir, sanitize_filename(file_obj.original_filename)
      ...
      def save_file
        ::FileUtils.mkpath DuckDescribe::FILE_DIR
        remove_file
        FileUtils.mv ::File.dirname(@tmp_file), DuckDescribe::FILE_DIR
      ...
      def remove_file
        ::FileUtils.rm_rf ::File.join(DuckDescribe::FILE_DIR, ::File.dirname(@value)) unless @value.blank?
        @value = nil
      end
